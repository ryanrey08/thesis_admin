	...
	<div class="container-fluid">
		...
		<div class="jumbotron">
			<div class="d-flex align-items-end justify-content-between">
				<h1>Staffs</h1>
				<button class="btn btn-success" trigger="hover" data-toggle="tooltip" title="Add New Staff" @click="addStaff">
					<i class="fas fa-user-plus white" aria-hidden="true"></i> Add New Staff
				</button>
			</div>
		</div>
					...
					<div class="col-sm-12">
						<div class="table-responsive">
							<table class="table table-hover no-border">
									...
									<tr>
										...
										<th colspan="2">
											<div class="input-group input-group-sm">
												...
												<input v-model="searchQuery" @keyup="searchStaff" class="form-control border border-0 rounded-right" placeholder="Search" aria-label="Search">
											</div>
										</th>
									</tr>
								</thead>
								...
								<tbody>
									<!-- <tr v-if="staffs.data.data.length == 0">
									...
									<tr v-for="staff in staffs.data" :key="staff.id">
										...
										<td class="text-center small">{{ staff.roledesc }}</td>
										<td align="text-center small" v-if="filter == 'Archived'">
											{{ staff.updated_at | cusDate }}
										</td>
										<td align="center" v-else>
											<span class="badge badge-pill" :class="[staff.isactive == 1 ? 'badge-success' : 'badge-secondary']">{{ staff.isactive == 1 ? "Active" : "Archived" }}</span>
										</td>
										<td class="text-center">
											<button class="btn btn-warning btn-sm" trigger="hover" data-toggle="tooltip" title="Edit Staff" v-if="staff.isactive == 1" @click="editStaff(staff)"><i class="fas fa-pencil-alt"></i></button>
	
											<button class="btn btn-danger btn-sm" trigger="hover" data-toggle="tooltip" title="Archive Staff" v-if="staff.isactive == 1" @click="deleteStaff(staff.id)"><i class="fas fa-trash"></i></button>

											<button class="btn btn-info btn-sm" trigger="hover" data-toggle="tooltip" title="Restore Staff" v-if="staff.isactive == 0" @click="restoreStaff(staff.id)"><i class="fas fa-trash-restore"></i></button>
										</td>
									</tr>
								</tbody>
							</table>
								...
								<div class="row">
									...
									<div class="col">
										...
										<div class="form-group">
											...
											<select v-model="staff.role_id" class="custom-select" name="role_id" id="role_id">
												<option v-for="role in roles" :value="role.id">{{ role.description }}</option>
											</select>

											<has-error :form="staff" field="role_id"></has-error>
	...
	export default {
		...
		computed: {
			filteredStaffs: function() {
				if(this.searchQuery) {
					return this.staffs.data.filter((staff) => {
		...
		methods: {
			...
			searchStaff: _.debounce(function() {
				axios.get('/api/staff/search?stf=' + this.searchQuery)
					.then(response => {
			...
			// 			return this.roles[index].description;
			// 		}
			// 	}
			// },
			...
			loadRoles: function() {
				...
				axios.get('/api/staff/roles').then(({data}) => (this.roles = data));
